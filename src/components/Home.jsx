...
export default function Home({ usuario, grupoAtivo, online = new Map(), versaoPc = false }) {
  ...
  function listaEquipe(pessoas, estaOnline) {
    if (pessoas.length === 0) return null
    return (
      <>
        <div className="equipe-secao-titulo">{estaOnline ? 'Online' : 'Offline'} — {pessoas.length}</div>
            ...
            <li key={p.id}>
              <button className="equipe-lista-item" onClick={() => setSelecionado(p)}>
                <span className="equipe-lista-avatar-wrap">
                  ...
                  <span className={`bolinha-status ${estaOnline ? 'bolinha-status-online' : 'bolinha-status-offline'}`} aria-label={estaOnline ? 'Online agora' : 'Offline'} />
                </span>
                <span className={`equipe-lista-nome ${estaOnline ? '' : 'equipe-lista-nome-offline'}`}>{p.nome}</span>
              </button>
            </li>
          ))}
        </ul>
  ...
  return (
    <div className="tela">
      <header className="topo topo-minimal topo-home">
      ...
      <section className="secao">
        {versaoPc ? (
          <>
            <div className="equipe-titulo-pc">
              <h3>Equipe</h3>
              <span className="equipe-card-count">{equipe.length}</span>
            </div>
            {listaEquipe(equipeOnline, true)}
            {listaEquipe(equipeOffline, false)}
          </>
        ) : (
